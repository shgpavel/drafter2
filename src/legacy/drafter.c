...
void drafter_init() {
  ...
  if (res != CURLE_OK) {
    fprintf(stderr,
            "Error: (drafter2) "
            "Failed to initialize global curl environment: "
...
void drafter_add_str_func() {
  ...
  curl = curl_easy_init();
  if (!curl) {
    fprintf(stderr, "Error: Failed to init curl request [drafter asf]\n");
    exit(1);
  }
...
void drafter_get_graph() {
  ...
  curl = curl_easy_init();
  if (!curl) {
    fprintf(stderr, "Error: Failed to init curl request [drafter gg]\n");
    exit(1);
  }
...
void drafter_clear_graph() {
  ...
  curl = curl_easy_init();
  if (!curl) {
    fprintf(stderr, "Error: Failed to init curl request [drafter cg]\n");
    exit(1);
  }

  struct curl_slist *headers = NULL;
  headers = curl_slist_append(headers, "Content-Type: application/json");
  curl_easy_setopt(curl, CURLOPT_URL, "http://localhost:3000/clear");
  curl_easy_setopt(curl, CURLOPT_POSTFIELDS, postfields);
  curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
